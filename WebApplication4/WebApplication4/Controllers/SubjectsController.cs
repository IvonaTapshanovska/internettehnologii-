...
namespace WebApplication4.Controllers
{
    public class SubjectsController : Controller
    {
        ...
        {
            StudentSubjectGrades model=new StudentSubjectGrades();
            model.SubjectId = id;
            model.students=db.Students.ToList();
            return View(model);
        }
        [HttpPost]
        public ActionResult AddGrade(StudentSubjectGrades model)
        {
            var subject=db.Subjects.Find(model.SubjectId);
            var student = db.Students.Find(model.selectedStudent);
